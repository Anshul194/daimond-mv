...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await dispatch(fetchSubCategoryById(subCategory));
      setSubCategoryData(res.payload || {});
      const response = await dispatch(fetchBlogBySubCategoryId(subCategory));
        ...
        <div className="relative z-10 flex items-center justify-center h-full">
          <div className="text-center text-white px-4">
            <h1 className="text-5xl md:text-5xl font-light font-arizona mb-2 tracking-wide">
              {subCategoryData.name || "Education"}
            </h1>
            <p className="text-lg md:text-sm font-gintoNord font-light">
              Discover Everything You Need to Know About {subCategoryData.name || "Fine Jewellery"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {data &&
            data.length > 0 &&
            data?.map((post, index) => (
              <Link href={`${path}/${post._id}`} key={index}>
